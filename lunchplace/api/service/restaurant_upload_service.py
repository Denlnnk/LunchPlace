...
def upload(request_data):
    ...
    only_files = [f for f in listdir(str(pathlib.Path().absolute()) + '/lunchplace/api/static/restaurants')]
    list_names = [names.rstrip('.json') for names in only_files]
    data = []
    requested_day = request_data['day']

    for restaurant_name in list_names:
        file = open(str(pathlib.Path().absolute()) + '/lunchplace/api/static/restaurants/' + restaurant_name + '.json')
